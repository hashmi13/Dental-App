...
async function sendBrevoTransactionalEmail({ toEmail, toName, templateId, params, subject, htmlContent }) {
  ...
  const senderEmail = process.env.BREVO_SENDER_EMAIL || "hashmifaizy4@gmail.com";
...
// IMPORTANT: Stripe Webhook endpoint (Raw body)
app.post(
  "/api/webhook",
...
app.post("/api/send-brevo-email", async (req, res) => {
  try {
    ...
    const result = await sendBrevoTransactionalEmail({ toEmail, toName, templateId, params });
...
app.post("/api/create-checkout-session", async (req, res) => {
  try {
    ...
    // Save pending appointment to MongoDB
    try {
      await connectDB();
...
app.get("/api/health", (req, res) => {
  ...
  res.json({
    status: "ok",
    message: "Vercel Serverless Express & MongoDB Backend is running!",
